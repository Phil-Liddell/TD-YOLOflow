...
def apply_augmentations(dataset_dir):
    ...
    # Apply augmentations (up to 3x original dataset size)
    max_augmentations = 3 * len(image_files) - len(image_files)
...
if __name__ == '__main__':
    ...
    model = YOLO('yolo11s.pt')  # Use yolo11s model
    ...
    model_name_base = os.path.splitext(os.path.basename('yolo11s.pt'))[0]
